...
Статус: перенос runtime-страниц и интерактивных сценариев на React завершён. Для code/evidence SHA `d0f99fe6dc64753d06544be79f658f82e5787062` применено Preview-only ACL/RPC hardening, полный schema preflight прошёл и новый exact immutable Preview подтверждён короткими route и disposable stored-XSS доказательствами. Полный durable acceptance и единственный formal load относятся к предыдущему runtime-under-test SHA `85f8e81fcc9849e0b679ceb51caa0816f9806f11`: acceptance зелёный, но load завершился `FAIL` из-за одного transport `ECONNRESET`; burst и soak не запускались. Поэтому release checkpoint остаётся **NO-GO**, повтор formal load запрещён, Production, `main` и merge не изменялись.
...
- GitHub topology не менялся: `main` остаётся на baseline `ee8476473b64de946d81dc1adbcd7dc3871e4ac9`; Draft PR [#1](https://github.com/Krevetkaaaa/mesto-react-migration/pull/1) по-прежнему открыт и не merged. Текущий code/evidence SHA — `d0f99fe6dc64753d06544be79f658f82e5787062`; его exact immutable Preview — deployment `dpl_681rfHsbnP9We7XGq4PqwjConqC7` по адресу `https://mesto-city-guide-pesr75f4x-krevetkaaaas-projects.vercel.app`. Deployment READY, `target=null`, exact SHA/provider fingerprint подтверждён; Production deployment и canonical alias до/после не изменились.
- Для runtime-under-test SHA `85f8e81fcc9849e0b679ceb51caa0816f9806f11` оба GitHub CI run зелёные с первой попытки: push `31965676156` и PR `31965678477`, по четыре успешных job. Для последующего hardening SHA `d0f99fe6dc64753d06544be79f658f82e5787062` локально прошли `check`, server tests `354/354`, Vitest `213/213`, targeted Supabase security/preflight tests `14/14`, `check:phase12` и `git diff --check`. GitHub API не позволил независимо прочитать CI этого SHA из-за ограничения аккаунта, поэтому terminal CI для `d0f99fe` здесь не заявляется.
- Короткие доказательства для acceptance runtime `85f8e81` остаются зелёными: strict accessibility — 9 проверок и 6 axe scans без serious/critical violations. После ACL migration новый exact Preview `d0f99fe` отдельно прошёл fingerprint и GET routes `7/7`, а disposable stored-XSS browser smoke — `3/3`, две reload-проверки, полный cleanup и нулевой остаток.
...
- Только в Preview применены и сверены с remote history все восемь migrations. Восьмая, `20260817092029_explicit_data_api_acl_and_function_hardening.sql`, применена ровно один раз через явно разрешённый Preview-only `execute_sql` fallback: все 63 canonical CLI statements и canonical history row записаны в одной транзакции под advisory lock. Независимая проверка подтвердила exact version/name/count/history digest. Repository-generated repeatable-read read-only schema preflight прошёл `19/19`; workflow guard: `publishing=0`, cleanup backlog `0`. Production migrations/data не изменялись.
...
- Первый acceptance для этого SHA был прерван teardown приложения и не классифицирован как terminal acceptance result. Его exact Preview-остатки затем очищены отдельным hash-bound recovery: cleanup terminal PASS, evidence validator и durable finalizer PASS, временная cleanup task удалена. Production при recovery не затрагивался.
- Ровно один последующий durable full acceptance завершился terminal `PASS`: scheduler runId `7c5c60caaa0b4fe880d83ccf38dc2d00`, acceptance-report runId `7364db2b0e55162e`, exit `0`, `passed:true`, `cleanup.complete:true`; cleanup failures отсутствуют, media cleanup/reaper доказательства завершены и backlog drained. Отдельный validator выдал `ACCEPTANCE_EVIDENCE_VALID`, durable finalizer — `DURABLE_R2_TERMINAL_EVIDENCE_VALID`, после чего acceptance task удалена.
- После persisted acceptance был запущен ровно один formal load с hard acceptance precondition. Он завершился exit `1`, `passed:false`; validator закономерно отклонил evidence как `LOAD_EXIT_NOT_ZERO`. Профиль expected выполнил 500 запросов: 499 HTTP 200, один transport `ECONNRESET`, 0 response-validation errors, 499 cache hits (`0,998`), `3813,65 ms`, `131,11 rps`, p50 `101,79 ms`, p95 `315,46 ms`, p99 `359,60 ms`. SLO провален только по строгому zero-error условию; профили burst и soak отмечены `notRun`. Повтор не был разрешён и не выполнялся.
- Post-migration advisors не нашли critical/high schema finding. До Production остаются hardening/debt пункты: mutable `search_path` у двух trigger helpers, выключенная leaked-password protection в Preview Auth, три RLS initplan performance warning и девять unindexed-FK informational finding. Service-only таблицы с RLS и без policies ожидаемы при полном запрете browser/public grants.
- Preview использует физически отдельный временный scratch Redis; он истекает 19 августа 2026 года и не является durable release-конфигурацией. Acceptance и load fail closed при shared/mismatched provider. Для нового release-доказательства после истечения ресурса потребуется durable isolated replacement и повтор соответствующих ворот.
- Production deployment `dpl_DXB5HmM7ZmdkEv9af6TUnxRxd6rg`, alias `mesto-city-guide.vercel.app`, Production data/migrations, `main` и merge не менялись. Promotion не авторизован.
- Кроме красного formal load внешними блокерами остаются durable isolated Redis, явное визуальное одобрение владельца, реальные field p75 Core Web Vitals, provider dashboard/trace evidence, репрезентативный non-production `EXPLAIN`, Production environment/preflight/shared-provider/cron, а также Production backup/PITR/migrations/rollback owner. Зелёные CI и acceptance не закрывают эти ворота.
