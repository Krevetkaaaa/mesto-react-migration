...
## Current Preview checkpoint — 2026-08-17

- This checkpoint supersedes only the current status above; historical contracts and evidence remain historical.
- Full acceptance and formal-load evidence remain bound to runtime-under-test SHA `85f8e81fcc9849e0b679ceb51caa0816f9806f11` and deployment `dpl_GyoXdtHoHiQqoUhjHjuHxvmMrwL9`. Its exact-SHA push run `31965676156` and PR run `31965678477` are green, with all four jobs successful on their first attempt.
- The current code/evidence SHA is `d0f99fe6dc64753d06544be79f658f82e5787062`. After Preview-only schema hardening, its exact immutable Preview is `dpl_681rfHsbnP9We7XGq4PqwjConqC7` at `https://mesto-city-guide-pesr75f4x-krevetkaaaas-projects.vercel.app`; it is READY with `target=null`, exact identity/fingerprint, and unchanged Production before/after. Local hardening verification passed `check`, server `354/354`, Vitest `213/213`, focused Supabase tests `14/14`, `check:phase12`, and diff check. GitHub CI for this later SHA is not claimed because the account API returned an access restriction.
- The acceptance runtime retains strict accessibility evidence: 9 checks and 6 axe scans with zero serious/critical violations. The post-migration `d0f99fe` Preview independently passed fingerprint plus GET routes `7/7`; one disposable stored-XSS browser smoke passed `3/3`, two reloads, complete cleanup, and zero residuals.
- Preview remote history now contains exactly eight migrations. `20260817092029_explicit_data_api_acl_and_function_hardening.sql` was applied once through the explicitly authorized Preview-only `execute_sql` fallback: 63 canonical CLI statements and the canonical history row committed in the same advisory-locked transaction. Exact version/name/count/digest validation passed. The repository-generated repeatable-read read-only schema preflight passed `19/19`, with `publishing=0` and cleanup backlog `0`. Production migrations/data were untouched.
- An interrupted earlier acceptance attempt was treated as non-terminal, then recovered through an exact hash-bound cleanup run. Cleanup evidence and its durable finalization both passed, and the temporary cleanup task was unregistered; no Production state was touched.
- Exactly one durable full acceptance then reached terminal PASS: scheduler runId `7c5c60caaa0b4fe880d83ccf38dc2d00`, acceptance-report runId `7364db2b0e55162e`, exit `0`, `passed:true`, `cleanup.complete:true`, no cleanup failures, and completed/drained media cleanup plus reaper proof. The evidence validator returned `ACCEPTANCE_EVIDENCE_VALID`; the finalizer returned `DURABLE_R2_TERMINAL_EVIDENCE_VALID` and unregistered the acceptance task.
- Exactly one formal load was subsequently run behind the hard acceptance precondition. It ended with exit `1`, `passed:false`, and validator result `LOAD_EXIT_NOT_ZERO`. The expected profile issued 500 requests: 499 HTTP 200, one transport `ECONNRESET`, zero response-validation errors, 499 cache hits (`0.998`), elapsed `3813.65 ms`, throughput `131.11 rps`, p50 `101.79 ms`, p95 `315.46 ms`, and p99 `359.60 ms`. It failed solely because the SLO requires zero errors; burst and soak are `notRun`. No rerun was authorized or performed.
- Post-migration advisors reported no critical/high schema finding. Remaining pre-Production hardening/debt is explicit: mutable search paths on two trigger helpers, disabled leaked-password protection in Preview Auth, three RLS initplan performance warnings, and nine unindexed-FK informational findings. RLS-enabled service-only tables without policies are intentional because browser/public table grants are absent.
- The isolated scratch Redis is temporary and expires on 2026-08-19; it is not a durable release configuration. A durable isolated replacement remains required for a future release proof.
- Production deployment `dpl_DXB5HmM7ZmdkEv9af6TUnxRxd6rg`, alias `mesto-city-guide.vercel.app`, Production data/migrations, `main` at `ee8476473b64de946d81dc1adbcd7dc3871e4ac9`, and merge remain unchanged. Release status is **NO-GO**. Remaining external gates include the red formal load, durable isolated Redis, owner visual approval, field p75 Core Web Vitals, provider dashboard/trace evidence, representative non-production `EXPLAIN`, Production environment/preflight/shared-provider/cron, and Production backup/PITR/migrations/rollback ownership.
