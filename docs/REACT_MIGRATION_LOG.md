...
- Verification: `npm.cmd run check`; `npm.cmd test` — `148/148` server и `189/189` unit; `npm.cmd run smoke`; 50 client assets без secret leakage; public home `508,4/152,1 KiB`; Phase 4 `23/52`, Phase 5 `35/80`, Phase 6 `28/104`, Phase 7 `64/128`, Phase 8 `57/105`; все числа — passed/expected skipped, exact Chrome, no snapshot update. Дополнительно вся Phase 8 прошла под server `TZ=UTC`, browser `Europe/Simferopol` без hydration errors.
