...
- React runtime migration в release-candidate code завершена. Проверенный и развёрнутый runtime SHA — `85f8e81fcc9849e0b679ceb51caa0816f9806f11`; immutable Preview — deployment `dpl_GyoXdtHoHiQqoUhjHjuHxvmMrwL9` по адресу `https://mesto-city-guide-a98xir82k-krevetkaaaas-projects.vercel.app`. Exact-SHA push/PR CI runs `31965676156` и `31965678477` зелёные, все четыре job в каждом прошли с первой попытки. Локальные `check`, server `340/340`, Vitest `213/213`, production smoke и high-severity audit gate также прошли.
- Короткие remote proofs зелёные: exact route/deployment identity; strict accessibility — 9 проверок и 6 axe scans без serious/critical violations, включая theme/OAuth/focus/mobile; disposable stored-XSS — `3/3`, две reload-проверки, cleanup complete и residual `0`.
