...
### Текущий release checkpoint — 13 августа 2026 года
...
- Release-candidate HEAD — `5427a689c47635751ca76ba543bb15318932ca1b`. В Draft PR [#1](https://github.com/Krevetkaaaa/mesto-react-migration/pull/1) все дублирующиеся CI checks зелёные после targeted browser rerun.
- Exact immutable Preview: deployment `dpl_6hdWFZ4jjV8CsuiHZCXVTeXwm8Es`, URL `https://mesto-city-guide-rzxk98vtr-krevetkaaaas-projects.vercel.app`.
- Изолированный Preview Supabase ref — `foxqdoyqcfsqngfotayu`; на нём применены и удалённо сверены все семь migrations.
- Fingerprint изолированного Preview Redis отличается от Production. Временный Free resource истекает 14 августа 2026 года и не является устойчивой release-конфигурацией.
- Первый долгий acceptance run потерян вместе с terminal channel и не считается доказательством. Persisted replacement run завершился 12 августа с exit `1`, `passed:false`, единственной workflow-ошибкой `cache.warm-a/CACHE_TAG_CONTRACT_MISSING` и `cleanup.complete:true` без cleanup failures. Media/reaper части прошли, stored-XSS browser oracle не успел запуститься. Причина — ложное ожидание публичного `Vercel-Cache-Tag`, который Vercel потребляет и удаляет из клиентского ответа. В working tree oracle заменён на строгий публичный `HIT(old) → STALE(old) → HIT(new)` с неизменным контрольным venue; до нового exact deployment и полного rerun checkpoint **не заявляет acceptance PASS**.
- Production deployment и alias не изменялись. Активный Production Supabase ref — `aelqtfzjvahpsdkabtny`, однако будущие Production environment, preflight, backup и migrations остаются нерешёнными.
- Remote load, полный stored-XSS browser proof, visual owner approval и Production promotion остаются pending. Зелёный CI и завершившийся красным Preview acceptance сами по себе не разрешают Production promotion.

### Superseding release checkpoint — 17 августа 2026 года

- Эта запись supersedes только текущие status/debt-утверждения checkpoint от 13 августа; вся предыдущая история, commits, deployments и результаты сохранены без переписывания.
- React runtime migration в release-candidate code завершена. Full acceptance/load evidence остаётся привязано к runtime-under-test SHA `85f8e81fcc9849e0b679ceb51caa0816f9806f11` и immutable Preview `dpl_GyoXdtHoHiQqoUhjHjuHxvmMrwL9`; его exact-SHA push/PR runs `31965676156` и `31965678477` зелёные. Текущий code/evidence SHA с Supabase ACL/RPC hardening — `d0f99fe6dc64753d06544be79f658f82e5787062`; его новый immutable Preview — `dpl_681rfHsbnP9We7XGq4PqwjConqC7` по адресу `https://mesto-city-guide-pesr75f4x-krevetkaaaas-projects.vercel.app`, READY и Preview-only. Production до/после не изменился.
- Для `d0f99fe` локально прошли `check`, server `354/354`, Vitest `213/213`, focused Supabase tests `14/14` и `check:phase12`. GitHub CI later SHA не заявляется: API вернул ограничение доступа аккаунта. Короткие post-migration remote proofs зелёные: exact fingerprint, GET routes `7/7`, disposable stored-XSS `3/3`, две reload-проверки, cleanup complete и residual `0`. Strict accessibility evidence 9/9 и 6 axe scans относится к acceptance runtime `85f8e81`.
- В Preview применена восьмая migration `20260817092029_explicit_data_api_acl_and_function_hardening.sql`: ровно один явно разрешённый atomic `execute_sql` fallback выполнил 63 canonical CLI statements и записал canonical history row в той же advisory-locked транзакции. Независимо подтверждены exact version/name/count/digest; repository-generated read-only schema preflight прошёл `19/19`, `publishing=0`, cleanup backlog `0`. Production DB не затрагивалась.
- Первый acceptance этого SHA был прерван teardown приложения и не считается terminal acceptance result. Его exact остатки затем безопасно удалены hash-bound cleanup recovery; cleanup evidence и durable finalization прошли, временная task удалена, Production не затрагивался.
- Ровно один последующий durable full acceptance (scheduler runId `7c5c60caaa0b4fe880d83ccf38dc2d00`, acceptance-report runId `7364db2b0e55162e`) завершился terminal `PASS`: exit `0`, `passed:true`, `cleanup.complete:true`, cleanup failures отсутствуют, media cleanup/reaper completed и drained. Validator вернул `ACCEPTANCE_EVIDENCE_VALID`, finalizer — `DURABLE_R2_TERMINAL_EVIDENCE_VALID`; acceptance task после фиксации evidence удалена.
- После этого был запущен ровно один formal load с hard acceptance precondition. Он завершился exit `1`, `passed:false`; validator вернул `LOAD_EXIT_NOT_ZERO`. Expected profile выполнил 500 запросов: 499 HTTP 200, один transport `ECONNRESET`, 0 response-validation errors, 499 cache hits (`0,998`), elapsed `3813,65 ms`, throughput `131,11 rps`, p50 `101,79 ms`, p95 `315,46 ms`, p99 `359,60 ms`. SLO провален исключительно строгим zero-error условием; burst и soak отмечены `notRun`. Повтор formal load не разрешался и не выполнялся.
- Post-migration advisors не нашли critical/high schema finding. До Production остаются mutable `search_path` у двух trigger helpers, disabled leaked-password protection в Preview Auth, три RLS initplan performance warning и девять unindexed-FK informational finding.
- Scratch Redis изолирован, но временный и истекает 19 августа 2026 года; durable replacement остаётся обязательным. Production deployment `dpl_DXB5HmM7ZmdkEv9af6TUnxRxd6rg`, alias `mesto-city-guide.vercel.app`, Production data/migrations, `main` `ee8476473b64de946d81dc1adbcd7dc3871e4ac9` и merge не изменялись.
- Итоговый release status — **NO-GO**. Кроме красного formal load незакрыты внешние ворота: durable isolated Redis, явное visual owner approval, реальные field p75 Core Web Vitals, provider dashboard/trace evidence, representative non-production `EXPLAIN`, Production environment/preflight/shared-provider/cron и Production backup/PITR/migrations/rollback owner. Зелёные CI и acceptance не разрешают merge или Production promotion.

### Superseding release checkpoint — 17 августа 2026 года, `a7bf625`

- Эта запись заменяет только текущий release status блока выше; предыдущие SHA, deployments, acceptance/load результаты и recovery остаются историческими доказательствами.
- Текущий runtime-under-test SHA — `a7bf625daccc37b58beb7e32928ad908a62f4288`; exact immutable Preview — `dpl_5ecdBNfn1WvBLkLh1HmD7U4ZDByh` по адресу `https://mesto-city-guide-4xtiis6s5-krevetkaaaas-projects.vercel.app`, READY и Preview-only. Remote branch указывает на exact SHA, remote `main` остаётся `ee8476473b64de946d81dc1adbcd7dc3871e4ac9`; GitHub API не позволяет независимо подтвердить более свежий terminal PR/CI status.
- Локально прошли `check`, server `357/357`, Vitest `213/213`, focused Supabase tests `17/17` и diff check. Preview прошёл fingerprint, GET routes `7/7`, strict accessibility 9 checks/6 axe scans и один disposable stored-XSS proof с 3 наблюдениями, 2 reload, полным cleanup и нулевым остатком.
- Preview remote history содержит девять migrations. `20260817121310_close_database_advisor_findings.sql` применён ровно один раз через явно разрешённый Preview-only atomic `execute_sql` fallback с canonical history row. Read-only schema preflight прошёл `21/21`, `publishing=0`, cleanup backlog `0`; Production DB не затрагивалась.
- Единственный full acceptance `a7bf625` выполнялся `7 682 891 ms` и завершился report `passed:false` с единственной workflow-ошибкой `media.public-hero/MEDIA_TRANSPORT_FAILURE`. `cleanup.complete:true`, supported cleanup complete, failures `0`; media gate passed, reaper completed/drained, изменяемых exact-run остатков нет. Пустой exit artifact не позволил выдать ложный PASS: validator вернул `ACCEPTANCE_EXIT_ARTIFACT_EMPTY`. Terminal task удалена, acceptance не перезапускался.
- Formal load для этого SHA не запускался, потому что hard acceptance precondition красный. Старый `85f8e81` load не повторялся. Production deployment/alias/data/migrations, `main` и merge не изменялись.
- Итоговый release status — **NO-GO**. Блокеры: current acceptance failure, истёкший Preview scratch Redis и отсутствие durable isolated replacement, disabled leaked-password protection, owner visual approval, field p75 Core Web Vitals, provider dashboard/trace evidence, representative non-production `EXPLAIN`, Production environment/preflight/shared-provider/cron и Production backup/PITR/migrations/rollback owner.
