...
- Безопасный upload fallback остаётся server-mediated: до Storage проверяются bytes, MIME/container signature, dimensions и лимиты. Direct signed staging upload, lifecycle cleanup, derivatives и immutable CDN verification не подменены небезопасной заглушкой и остаются внешним Phase 9 пунктом.
...
Текущий tree не объявляется production-like или production готовым.
