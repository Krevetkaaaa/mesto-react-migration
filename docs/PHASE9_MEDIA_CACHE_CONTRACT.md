...
- Full acceptance/load evidence привязано к runtime-under-test SHA `85f8e81fcc9849e0b679ceb51caa0816f9806f11` и deployment `dpl_GyoXdtHoHiQqoUhjHjuHxvmMrwL9`; его push/PR CI runs `31965676156` и `31965678477` зелёные. Текущий code/evidence SHA после Supabase hardening — `d0f99fe6dc64753d06544be79f658f82e5787062`; его exact immutable Preview — `dpl_681rfHsbnP9We7XGq4PqwjConqC7` по адресу `https://mesto-city-guide-pesr75f4x-krevetkaaaas-projects.vercel.app`, READY, `target=null`, Production unchanged.
- Для `d0f99fe` локально прошли `check`, server `354/354`, Vitest `213/213`, focused Supabase security/preflight `14/14` и `check:phase12`. После Preview migration fingerprint и GET routes прошли `7/7`; disposable stored-XSS browser proof прошёл `3/3`, две reload-проверки, полный cleanup и остаток `0`. CI этого later SHA не заявляется из-за ограничения доступа GitHub API.
- Preview remote history содержит ровно восемь migrations. ACL/RPC hardening `20260817092029` применён один раз как явно разрешённый Preview-only atomic fallback: 63 canonical statements и canonical history row в одной advisory-locked транзакции. Exact history validation и read-only schema preflight `19/19` прошли; `publishing=0`, cleanup backlog `0`. Production data/migrations не изменялись.
...
- Post-migration advisors не обнаружили critical/high schema finding. До Production остаются два trigger-helper `search_path` warning, disabled leaked-password protection, три RLS initplan performance warning и девять unindexed-FK info finding; они не переопределяют красный formal-load gate.
