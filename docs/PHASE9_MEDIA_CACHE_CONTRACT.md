...
### Текущий verification/status checkpoint — 13 августа 2026 года

- Этот checkpoint заменяет только текущий status; исторические implementation checkpoints выше сохранены как история.
- Release-candidate HEAD — `5427a689c47635751ca76ba543bb15318932ca1b`. В Draft PR #1 все дублирующиеся CI checks зелёные после targeted browser rerun.
- Exact immutable Preview — deployment `dpl_6hdWFZ4jjV8CsuiHZCXVTeXwm8Es` по адресу `https://mesto-city-guide-rzxk98vtr-krevetkaaaas-projects.vercel.app`.
- Изолированный Preview Supabase ref — `foxqdoyqcfsqngfotayu`; все семь migrations применены и удалённо сверены. Fingerprint изолированного Preview Redis отличается от Production, но временный Free resource истекает 14 августа 2026 года.
- Первый долгий acceptance run был потерян вместе с terminal channel и не считается доказательством. Persisted replacement run завершился 12 августа с exit `1`, `passed:false`, единственной workflow-ошибкой `cache.warm-a/CACHE_TAG_CONTRACT_MISSING` и полным cleanup без ошибок. Media pipeline и exact reaper proof прошли, но stored-XSS browser oracle не запускался: workflow остановился до создания menu/promotion. Причина оказалась в ошибочном ожидании публичного `Vercel-Cache-Tag`, который Vercel штатно потребляет и удаляет из клиентского ответа. Исправленный oracle требует публичную последовательность `HIT(old) → STALE(old) → HIT(new)`, совпадение PoP/старого ETag и неизменный явный `HIT` у контрольного venue. Нужны новый exact deployment и полный повторный acceptance; PASS не заявляется.
- Production deployment и alias не изменялись. Активный Production Supabase ref — `aelqtfzjvahpsdkabtny`; будущие Production environment, preflight, backup и migrations пока не решены.
- Remote load, browser/visual owner approval и Production promotion остаются pending. До их завершения текущий Preview не является основанием для Production promotion.
