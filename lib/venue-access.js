...
function venueIdsForPermission(workspace, permission) {
  return workspace.venueIds.filter((venueId) => hasPermission(workspace, venueId, permission));
}

function requireVenue(workspace, res, venueId, permission) {
  if (workspace.profile?.must_change_password) {
    json(res, 403, { message: 'Сначала замените временный пароль.', code: 'PASSWORD_CHANGE_REQUIRED' });
    return null;
  }
...
module.exports = { hasPermission, membershipPermissions, merchantWorkspace, requireVenue, venueIdsForPermission };
