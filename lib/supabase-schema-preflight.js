...
const SUPPORTING_INDEXES = Object.freeze([
  ['audit_log_actor_id_idx', 'audit_log', 'actor_id', 'actor_idisnotnull'],
  ['favorites_venue_id_idx', 'favorites', 'venue_id', 'venue_idisnotnull'],
  ['review_submissions_submitted_by_idx', 'review_submissions', 'submitted_by', 'submitted_byisnotnull'],
  ['review_submissions_venue_id_idx', 'review_submissions', 'venue_id', 'venue_idisnotnull'],
  ['reviews_author_id_idx', 'reviews', 'author_id', 'author_idisnotnull'],
  ['reviews_venue_id_idx', 'reviews', 'venue_id', 'venue_idisnotnull'],
  ['venue_submissions_submitted_by_idx', 'venue_submissions', 'submitted_by', 'submitted_byisnotnull'],
  ['venues_created_by_idx', 'venues', 'created_by', 'created_byisnotnull'],
  ['venues_owner_id_idx', 'venues', 'owner_id', 'owner_idisnotnull']
]);
...
const REQUIRED_ADVISOR_MIGRATION_VERSION = '20260817121310';
...
function readMigrationVersions({ rootDirectory, migrationNames } = {}) {
  ...
  if (!versions.includes(REQUIRED_ADVISOR_MIGRATION_VERSION)) {
    throw new Error(`Required Supabase advisor migration ${REQUIRED_ADVISOR_MIGRATION_VERSION} is missing.`);
  }
  return versions;
}
...
function buildSupabaseSchemaPreflightSql({ migrationVersions }) {
  ...
  const supportingIndexRows = SUPPORTING_INDEXES.map(([name, table, columns, predicate]) => [
    name,
    table,
    columns,
    predicate,
    `createindex${name}onpublic.${table}usingbtree${columns}where${predicate}`
  ]);
...
BEGIN
  ...
  IF EXISTS (
    WITH expected(index_name,table_name,column_names,normalized_predicate,normalized_definition) AS (VALUES
      ${valuesSql(supportingIndexRows)}
    )
    SELECT 1 FROM expected
    LEFT JOIN pg_class i ON i.relnamespace = 'public'::regnamespace AND i.relname::text = expected.index_name
    LEFT JOIN pg_index x ON x.indexrelid = i.oid
    ...
    WHERE i.oid IS NULL OR x.indrelid IS DISTINCT FROM format('public.%I', expected.table_name)::regclass
      OR x.indisunique OR NOT x.indisvalid OR NOT x.indisready OR NOT x.indimmediate
      OR x.indisexclusion OR am.amname <> 'btree' OR x.indexprs IS NOT NULL
      OR actual.column_names IS DISTINCT FROM expected.column_names
      OR coalesce(regexp_replace(lower(pg_get_expr(x.indpred,x.indrelid)), '[()[:space:]]', '', 'g'),'')
        IS DISTINCT FROM expected.normalized_predicate
      OR regexp_replace(lower(pg_get_indexdef(i.oid)), '[()[:space:]\"]', '', 'g')
        IS DISTINCT FROM expected.normalized_definition
  ) THEN RAISE EXCEPTION 'MESTO_SCHEMA_PREFLIGHT_SUPPORTING_INDEXES'; END IF;

  IF EXISTS (
    WITH expected(table_name,policy_name,command,column_name,requires_check) AS (VALUES
      ('profiles','Users can read own profile','r','id',false),
      ('favorites','Users manage own favorites','*','user_id',true),
      ('venue_memberships','Merchants read own memberships','r','user_id',false)
    )
    SELECT 1 FROM expected
    LEFT JOIN pg_policy p
      ON p.polrelid = format('public.%I', expected.table_name)::regclass
      AND p.polname::text = expected.policy_name
    WHERE p.oid IS NULL OR p.polcmd::text IS DISTINCT FROM expected.command
      OR p.polroles IS DISTINCT FROM ARRAY[(SELECT oid FROM pg_roles WHERE rolname = 'authenticated')]
      OR position('selectauth.uid' in regexp_replace(lower(pg_get_expr(p.polqual,p.polrelid)), '[()[:space:]]', '', 'g')) = 0
      OR position(expected.column_name in regexp_replace(lower(pg_get_expr(p.polqual,p.polrelid)), '[()[:space:]]', '', 'g')) = 0
      OR (expected.requires_check AND (
        p.polwithcheck IS NULL
        OR position('selectauth.uid' in regexp_replace(lower(pg_get_expr(p.polwithcheck,p.polrelid)), '[()[:space:]]', '', 'g')) = 0
        OR position(expected.column_name in regexp_replace(lower(pg_get_expr(p.polwithcheck,p.polrelid)), '[()[:space:]]', '', 'g')) = 0
      ))
  ) THEN RAISE EXCEPTION 'MESTO_SCHEMA_PREFLIGHT_RLS_POLICIES'; END IF;
  ...
  IF EXISTS (
    ...
    WHERE to_regprocedure(expected.signature) IS NULL
      ...
      OR coalesce((
        SELECT split_part(config,'=',2) FROM pg_proc p, unnest(p.proconfig) config
        WHERE p.oid = to_regprocedure(expected.signature) AND split_part(config,'=',1) = 'search_path'
      ), 'missing') NOT IN ('','""')
      OR EXISTS (
