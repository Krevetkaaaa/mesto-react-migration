  ...
  return {
    ...
    async saveMenuItem(payload, id, userId) {
      try {
        ...
        if (userId) {
          ...
          if (id && current.some((entry) => entry.id === id)) await updateAuthMetadata(userId, { app_metadata: { mesto_menu: current.filter((entry) => entry.id !== id) } });
        }
        return item;
      } catch (error) {
        if (!missingExtendedSchema(error) || !userId) throw error;
    ...
    async deleteMenuItem(id, userId) {
      ...
      if (userId) {
        const user = await authUserById(userId);
        const current = Array.isArray(user.app_metadata?.mesto_menu) ? user.app_metadata.mesto_menu : [];
        if (current.some((item) => item.id === id)) await updateAuthMetadata(userId, { app_metadata: { mesto_menu: current.filter((item) => item.id !== id) } });
      }
      return removed;
    },
    ...
    async savePromotion(payload, id, userId) {
      try {
        ...
        if (userId) {
          ...
          if (id && current.some((entry) => entry.id === id)) await updateAuthMetadata(userId, { app_metadata: { mesto_promotions: current.filter((entry) => entry.id !== id) } });
        }
        return item;
      } catch (error) {
        if (!missingExtendedSchema(error) || !userId) throw error;
    ...
    async deletePromotion(id, userId) {
      ...
      if (userId) {
        const user = await authUserById(userId);
        const current = Array.isArray(user.app_metadata?.mesto_promotions) ? user.app_metadata.mesto_promotions : [];
        if (current.some((item) => item.id === id)) await updateAuthMetadata(userId, { app_metadata: { mesto_promotions: current.filter((item) => item.id !== id) } });
      }
      return removed;
    },
    async audit(entry) {
