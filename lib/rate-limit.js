...
function createUpstashRedisRateLimitAdapter({ url, token, fetchImpl = global.fetch } = {}) {
    ...
    async consume({ key, limit, windowMs }) {
      ...
      try {
        response = await fetchImpl(config.endpoint, {
          ...
          body: JSON.stringify(['EVAL', REDIS_SCRIPT, '1', `mesto:rate-limit:${key}`, String(limit), String(windowMs)]),
...
function configuration(environment = process.env) {
  ...
  if (provider === 'upstash-redis') {
    const validated = validateRedisConfiguration({
      url: environment.MESTO_RATE_LIMIT_REDIS_URL,
      token: environment.MESTO_RATE_LIMIT_REDIS_TOKEN
    });
