...
function redirect(res, target, statusCode = 302) {
...
function canonicalOAuthStartUrl(req, provider) {
  const origin = publicOrigin();
  const configuredHost = new URL(origin).host.toLowerCase();
  const rawHost = req?.headers?.host;
  const requestHost = String(Array.isArray(rawHost) ? rawHost[0] : rawHost || '').trim().toLowerCase();
  if (requestHost === configuredHost) return '';
  const target = new URL('/api/auth/oauth', origin);
  target.searchParams.set('provider', provider);
  const returnTo = strictReturnTarget(queryValue(req?.query?.returnTo, '/'));
  if (returnTo !== '/') target.searchParams.set('returnTo', returnTo);
  return target.href;
}

function redirectToCanonicalOAuthStart(req, res, provider) {
  const target = canonicalOAuthStartUrl(req, provider);
  if (!target) return false;
  redirect(res, target, 307);
  return true;
}
