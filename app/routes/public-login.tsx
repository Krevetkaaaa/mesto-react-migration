import type { Route } from "./+types/public-login";
...
export function action({ request }: Route.ActionArgs) {
  return performAuthAction(request, "login");
}
...
export function meta() {
  return [
    { title: "Войти — Место" },
    { name: "description", content: "Вход в личный кабинет городского гида «Место»." },
...
export function ErrorBoundary() {
  return <PublicHomeMarkup standalone homeBackdrop routeKind="login" accountDialogContent={<AccountRouteFailure />} />;
}

export default function PublicLogin({ actionData, loaderData }: Route.ComponentProps) {
  return (
    <PublicHomeMarkup
      standalone
      homeBackdrop
      routeKind="login"
      accountDialogContent={<AuthDialog actionData={actionData} mode="login" oauthCallbackError={loaderData.oauthError} providers={loaderData.providers} returnTo={loaderData.returnTo} />}
    />
  );
}
